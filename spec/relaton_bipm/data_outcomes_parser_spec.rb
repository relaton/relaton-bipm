describe RelatonBipm::DataOutcomesParser do
  ...
  context "instance methods" do
    ...
    let(:data_fetcher) do
      double "data_fetcher", output: "data", format: "yaml", ext: "yaml", files: [],
                             index2: index2
    end
    ...
    it "#fetch_type" do
      ...
      expect(subject).to receive(:fetch_meeting).with("cgpm/meetings-en/1.yml",
                                                      "CGPM", "Meeting", "data/cgpm/meeting")
    ...
    describe "#contributors" do
      shared_examples "contributors" do |date, body|
        it do
          ...
          expect(contribs[0][:entity][:abbreviation]).to eq content: "BIPM",
                                                            language: ["en", "fr"], script: "Latn"
      ...
      it_behaves_like "contributors", "1998-11-11", "CCTF"
      it_behaves_like "contributors", "1999-11-11", "CCTF"
    end

    describe "#fetch_meeting" do
      it "no part" do
        ...
        expect(index2).to receive(:add_or_update).with(
          { group: "CGPM", type: "Meeting", number: "1",
            year: "1889" }, "data/cgpm/meeting/1.yaml"
        )
        subject.fetch_meeting "spec/fixtures/cgpm/meetings-en/meeting-01.yml",
                              "CGPM", "Meeting", "data/cgpm/meeting"
      end

      it "with part" do
        allow(File).to receive(:read).and_wrap_original do |method, f, **args|
          file = if f == "data/cipm/meeting/101.yaml"
                   "spec/fixtures/#{f.sub(
                     '101', '101_1'
                   )}"
                 else
        ...
        expect(index2).to receive(:add_or_update).with(
          { group: "CIPM", type: "Meeting", number: "101-1",
            year: "2012" }, "data/cipm/meeting/101-1.yaml"
        )
        expect(index2).to receive(:add_or_update).with(
          { group: "CIPM", type: "Meeting", number: "101-2",
            year: "2012" }, "data/cipm/meeting/101-2.yaml"
        )
        expect(index2).to receive(:add_or_update).with(
          { group: "CIPM", type: "Meeting", number: "101",
            year: "2012" }, "data/cipm/meeting/101.yaml"
        )
        subject.fetch_meeting "spec/fixtures/cipm/meetings-en/meeting-101-1.yml",
                              "CIPM", "Meeting", "data/cipm/meeting"
        subject.fetch_meeting "spec/fixtures/cipm/meetings-en/meeting-101-2.yml",
                              "CIPM", "Meeting", "data/cipm/meeting"
      end
      ...
      it "without fr" do
        ...
        expect(index2).to receive(:add_or_update).with(
          { group: "JCRB", type: "Meeting", number: "48",
            year: "2024" }, "#{outdir}/48.yaml"
        )
        expect(index2).to receive(:add_or_update).with(
          { group: "JCRB", type: "ACT", number: "48-1",
            year: "2024" }, "#{outdir}/action/2024-48-1.yaml"
        )
        expect(index2).to receive(:add_or_update).with(
          { group: "JCRB", type: "ACT", number: "48-2",
            year: "2024" }, "#{outdir}/action/2024-48-2.yaml"
        )
        expect(index2).to receive(:add_or_update).with(
          { group: "JCRB", type: "ACT", number: "48-3",
            year: "2024" }, "#{outdir}/action/2024-48-3.yaml"
        )
        expect(index2).to receive(:add_or_update).with(
          { group: "JCRB", type: "RES", number: "48-1",
            year: "2024" }, "#{outdir}/resolution/2024-48-1.yaml"
        )
        expect(index2).to receive(:add_or_update).with(
          { group: "JCRB", type: "RES", number: "48-2",
            year: "2024" }, "#{outdir}/resolution/2024-48-2.yaml"
        )

        subject.fetch_meeting "spec/fixtures/jcrb/meetings-en/meeting-48.yml",
                              "JCRB", "Meeting", outdir
      end
    end
    ...
    describe "#parse_file" do
      let(:data_fetcher) do
        double "data_fetcher", output: "data", format: "xml", ext: "xml", files: [],
                               index2: index2
      end

      it "xml" do
        expect(File).to receive(:read).with("file.xml",
                                            encoding: "UTF-8").and_return "xml"
        expect(RelatonBipm::XMLParser).to receive(:from_xml).with("xml").and_return "item"
        expect(subject.parse_file("file.xml")).to eq "item"
      end
    end

    describe "#fetch_resolution" do
      it "one resolution" do
        ...
        src = [{ type: "src",
                 content: "http://www.bipm.org/publications/cgpm/meeting-01.html" }]

        expect(index2).to receive(:add_or_update).with(
          { group: "CGPM", type: "RES",
            year: "1889" }, "data/cgpm/meeting/resolution/1889-00.yaml"
        )
      ...
      it "multiple resolutions" do
        ...
        src = [{ type: "src",
                 content: "http://www.bipm.org/publications/cipm/meeting-01.html" }]

        expect(index2).to receive(:add_or_update).with(kind_of(Hash),
                                                       kind_of(String)).exactly(40).times
        subject.fetch_resolution(
    ...
    describe "#resolution_fr_long_id" do
      ...
      it_behaves_like "resolution_fr_long_id", "CIPM", "Decision", "10-1",
                      "2012", "Décision CIPM/10-1 (2012)"
      it_behaves_like "resolution_fr_long_id", "CIPM", "Resolution", "1",
                      "2012", "Résolution 1 du CIPM (2012)"
      it_behaves_like "resolution_fr_long_id", "CGPM", "Resolution", "1",
                      "2012", "Résolution 1 de la CGPM (2012)"
      it_behaves_like "resolution_fr_long_id", "CIPM", "NoTranslation", "",
                      "2012", "NoTranslation du CIPM (2012)"
    end
  end
end
